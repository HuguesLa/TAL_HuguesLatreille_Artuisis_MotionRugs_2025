...
public class CSVDataLoader {
    ...
    private static String lastUsedDir = "H:\\\\Documents\\\\TAL\\\\artuisis-main\\\\artuisis-main\\\\Assets\\\\Results";

    public static void checkAndLoadCSVDataSets(String[] datapath) {
        File folder;
        String localdir = "H:\\Documents\\TAL\\artuisis-main\\artuisis-main\\Assets\\Results";
        if (datapath != null && datapath.length > 0) {
            localdir = datapath[0];
            lastUsedDir = localdir;
            folder = new File(localdir);
        } else {
            if (datapath == null) {
                localdir = lastUsedDir;
            }
    ...
    /**
     * Recharge les datasets après avoir effacé les précédents
     * @param datapath 
     */
    public static void reloadCSVDataSets(String[] datapath) {
        // Effacer les datasets existants
        SessionData.getInstance().clearDatasets();
        
        // Recharger les datasets
        checkAndLoadCSVDataSets(datapath);
    }
    ...
    public static List<String> listCSVFiles(String[] datapath) {
        File folder;
        String localdir = lastUsedDir;
        ...
        try {
            System.out.println("Scanning directory: " + folder.getCanonicalPath());
        } catch (IOException ex) {
            Logger.getLogger(CSVDataLoader.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    public static void loadSelectedFile(String fileName) {
        File file = new File(lastUsedDir + File.separator + fileName);
        if (file.exists()) {
            loadFile(file);
        } else {
